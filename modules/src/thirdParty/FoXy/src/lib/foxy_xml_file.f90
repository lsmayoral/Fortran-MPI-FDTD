!< FoXy XML  type(xml_tag),   intent(in)    :: tag        !< XML tag. file class.
...
type, public:: xml_file
  ...
  contains
    ! public methods
    procedure :: get_Nt     !< Get number of XML tags
    procedure :: get_tags    !< Get XML tags array
...
contains
  ! public methods
  pure function get_Nt(self) result(n)
  ...
  endsubroutine free
    ...
    subroutine get_tags(self,tags)
        class(xml_file) :: self 	!< XML file.
        type(xml_tag),allocatable,intent(out):: tags(:)  !< XML tag array
        allocate(tags(1:self%Nt))
        tags=self%tag
    endsubroutine get_tags
