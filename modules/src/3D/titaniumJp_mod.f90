...
!**************************************************************
!		Parameter of 3 Lorenztian model for Ti to fit from 600nm to 1000nm
!**************************************************************
	integer :: npols=4
	real*8 :: epinf=1.1d0
	real*8, dimension(4) :: xi1=(/ -12.74873d0,0.9605442d0,67.66233d0,55.0525d0 /)
	real*8, dimension(4) :: w0l=(2.d0*pi*c0/micro)*(/ 4.36014d0,-1.261863d0,0.4063879d0,-2.176176d0 /)
	real*8, dimension(4) :: dnu0l=(2.d0*pi*c0/micro)*(/ -5.825585d0,0.2087635d0,1.246245d0,8.059584d0 /)

	    contains
	    procedure :: update_EJp => update_E_inTi
	    procedure :: initialize_material => initialize_Ti
	    procedure :: initialize_Ti
	    procedure :: allocate_TiVars
	end type Ti_LJp

	contains 

subroutine update_E_inTi(this,Ex_pointer,Ey_pointer,Ez_pointer,Hx_pointer,Hy_pointer,Hz_pointer)
	    class(Ti_LJp) :: this
	...
	end subroutine update_E_inTi

	subroutine initialize_Ti(this,rangex,rangey,rangez,range_localdim)
	    class(Ti_LJp),intent(inout) :: this
	    ...
	    if(this%xrange%init.gt.0.and.this%xrange%ends.gt.0.and.&
	       this%yrange%init.gt.0.and.this%yrange%ends.gt.0.and.&
	       this%zrange%init.gt.0.and.this%zrange%ends.gt.0)then
		    call allocate_TiVars(this,range_localdim)
		    call initialize_TiVars(this)
	    endif
        end subroutine initialize_Ti

	subroutine allocate_TiVars(this,range_localdim)
	    class(Ti_LJp) :: this
	    integer,dimension(3),intent(in) :: range_localdim
	...
	end subroutine allocate_TiVars

	subroutine initialize_TiVars(this)
	class(Ti_LJp) :: this
...
!		print *,l,"gamma1L=",this%gamma1L(l),"gamma2L=",this%gamma2L(l)
	    ...
	    enddo
	enddo
	end subroutine initialize_TiVars

	end module titaniumJp_mod